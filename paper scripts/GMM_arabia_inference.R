...
for (m_K in 1:length(Kvect)) {
  K <- Kvect[m_K]
  save_file <- file.path("GMM/outputs/K_final", paste0("arabia_K",K,".rds"))
  if(!file.exists(save_file)) {
...
yaml::write_yaml(loo_summary, file.path("GMM/outputs/K_final","loo_summary.yaml"))

# Do the Baysian summary for the best model
density_model <- density_model0
density_model$K <- K_best
save_file <- file.path("MGMM/outputs/K_final", paste0("arabia_K",K_best,".rds"))
bayesian_soln <- readRDS(save_file)
...
write_rds(bayesian_df_KX,"GMM/outputs/K_final/arabia_dens_Kbest.rds")
