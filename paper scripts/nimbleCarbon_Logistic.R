...
bins = binPrep(sites=arabia_data$SiteID,ages=arabia_data$CRA,h=100) 
# Sample 1 date from each bin, selecting randomly the sample with the smallest error
caldates = caldates[thinDates(ages=arabia_data$CRA,  errors=arabia_data$Error, bins=bins, size=1, thresh=1,seed=123,method='splitsample')]
#splitsample or random can be used for 'method'. splitSample chooses the first 60% from the lowest errors then fills in the rest with random selection.

#check the sensitivity of the bin width (h).
#binsense(x=caldates,y=arabia_data$SiteID,h=seq(0,500,50),timeRange=c(5200,3200))
...
write_rds(MCMC_log_posteriors,"Master Paper/rds/MCMC_Logistic_posteriors.rds")
...
set.seed(123)
pp.check.mL.a=postPredSPD(obs.CRA,errors = obs.Errors,calCurve = 'intcal20',model = dLogisticGrowth,a = 5200,b=3200,params=list(r=mcmc.samples.mL.a$samples$chain2[,'r'],k=mcmc.samples.mL.a$samples$chain2[,'k']),method='uncalsample',nsim = 100,ncores = 2,verbose=FALSE)
par(mfrow=c(1,1))
...
Post.Pred.Check = apply(pp.check.mL.a$simmatrix, 1, quantile, c(0,.1,.5,.9,1))
...
write_rds(Post.Pred.Check.Log,"Master Paper/rds/Logistic_post_pred_check.rds")
